...
const config = {
  ...
  // The name of the current stack, such as 'dev'
  stack: pulumi.getStack(),
};

console.log("Stack is " + config.stack);

// contentBucket is the S3 bucket that the website's contents will be stored in.
const contentBucket = new aws.s3.Bucket(
  `${config.targetDomain}-bucket-${config.stack}`,
  {
    bucket: `${config.targetDomain}-bucket-${config.stack}`,
    acl: "public-read",
...
crawlDirectory(webContentsRootPath, (filePath: string) => {
  ...
  const contentFile = new aws.s3.BucketObject(
    relativeFilePath,
    {
      ...
      acl: "public-read", // must be public read as public website
...
const logsBucket = new aws.s3.Bucket(
  `${config.targetDomain}-logs-${config.stack}`,
  {
    bucket: `${config.targetDomain}-logs-${config.stack}`,
    acl: "private",
  }
);
...
if (config.certificateArn === undefined) {
  const eastRegion = new aws.Provider("us-east-1-region", {
...
const cdn = new aws.cloudfront.Distribution(
  `${config.targetDomain}-cdn-${config.stack}`,
  distributionArgs
);
